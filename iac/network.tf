...
resource "azurerm_subnet_route_table_association" "app_service_plan_subnet" {
  subnet_id      = azurerm_subnet.app_service_plan.id
  route_table_id = azurerm_route_table.app_service_plan_subnet.id
}
...
locals {
  private_endpoints_subnet_nsg_rules = {
    "AllowWebApp" = {
      ...
      protocol                   = "Tcp"
      access                     = "Allow"
      source_address_prefix      = var.asp_subnet_address_prefixes
