...
resource "terraform_data" "assign_fa_identity_role" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      psql -h ${azurerm_postgresql_flexible_server.nomad.fqdn} -p 5432 -U ${data.azuread_user.current_user.user_principal_name} -d postgres -f ${local.postgres_assign_ad_role_script_path}
      EOT
